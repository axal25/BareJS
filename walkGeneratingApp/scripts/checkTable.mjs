...
function checkArrayValidity( array ) {
    if(array === "Not Generated Yet.") {
        ...
        if (array instanceof Array) {
            return checkDirectionsWithMessages( array );
        } else throw new Error("Function's checkArrayValidity( array ) 1st argument is not an Array. " + "typeof array = " + (typeof array) + ". array = " + array + ".");
    }
}
...
const checkDirections = directionsArray => {
    ...
    console.log(`before if( !isProperFinishPoint( directionsArray ) ) return false;`);
    if( !isProperFinishPoint( directionsArray ) ) return false;
    console.log(`after if( !isProperFinishPoint( directionsArray ) ) return false;\nSo TRUE.`);
    return true;
};
...
const isProperFinishPoint = ( directionsArray ) => {
    let finishCoordinates = getFinishPoint( directionsArray );
    return finishCoordinates.x === 0 & finishCoordinates.y === 0;
};
